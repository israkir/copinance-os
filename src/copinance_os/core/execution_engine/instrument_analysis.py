...
class InstrumentAnalysisExecutor(BaseAnalysisExecutor):
    ...
    def _normalize_quote(self, quote: dict[str, Any], symbol: str) -> dict[str, Any]:
        ...
        return {
            ...
            "open": str(quote.get("open") or 0),
            "high": str(quote.get("high") or 0),
            "low": str(quote.get("low") or 0),
    ...
    def _historical_data_to_summary(
        ...
        sorted_pts = coerce_sorted_market_data_points(data_points)
        price_series_payload: dict[str, Any] | None = None
        if sorted_pts:
            ps = price_series_from_market_data_points(sorted_pts)
            price_series_payload = ps.model_dump(mode="json")
