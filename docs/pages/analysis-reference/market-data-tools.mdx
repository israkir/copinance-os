...
Options chain for an underlying: expiration dates, calls, puts, strike, last price, implied volatility, open interest, volume, and BSM Greeks (QuantLib is a core dependency). See [Options & Greeks](options-and-greeks) for how Greeks are computed.
...
**Parameters:** `symbol` (required, underlying ticker); optional **`window`**: `near` (default) or `mid`. `near` uses expiries with **0–21** days to expiry; `mid` uses **21–75** DTE (up to two dates in-band; falls back to the nearest two listed expiries if the band is empty). Same selection as [Library — Options positioning context](../getting-started/library#options-positioning-context).
...
These tools resolve **fund series / companies / share classes**, not common-stock 10-Ks.

### `find_sec_funds`

Search SEC-registered funds by name fragment (`search_type`: `series` / `company` / `class`). Use an id from a row with the tools below.

### `get_sec_fund_entity`

Resolve a fund ticker, series id, or CIK to hierarchy (company, series, classes).

### `get_sec_fund_filings`

Fund filings for a resolved entity (default form **NPORT-P**). Optional `series_only` narrows via EFTS.

### `get_sec_fund_portfolio`

Holdings from a fund NPORT-P (or related) report.

### `get_sec_fund_latest_report`

Latest available portfolio-style report for a fund identifier.
