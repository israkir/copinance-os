...
- **Macro indicators** aggregate observable economic series — rates, credit, labor, housing, manufacturing, consumer, global — into structured signals. About **38 FRED series** when a FRED key is set, plus yfinance proxies (ETFs, FX) where FRED is unavailable.
...
Aggregates VIX level, market breadth, and sector rotation into a composite regime signal for the broad market. This tool is **not** part of `create_rule_based_regime_tools`; it is registered separately for question-driven and `analyze macro` pipelines.
...
# tools: detect_market_trend, detect_volatility_regime, detect_market_cycles
# VIX / breadth / rotation: get_market_regime_indicators (separate class)
```
...
The **`get_macro_regime_indicators`** tool aggregates about 38 FRED series into nine categories (plus yfinance proxies when FRED is missing). Numbers are fetched via `MacroeconomicDataProvider` (FRED when configured, yfinance otherwise) — the LLM interprets tool output, it does not compute economic prints.
...
Industrial production (INDPRO), capacity utilization (TCU), manufacturing IP (IPMAN), durable goods orders (NEWORDER), factory orders (AMTMTI), manufacturing employment (DMANEMP).
