...
The question-driven registry also exposes **`get_options_positioning`** ([market data tools](../analysis-reference/market-data-tools#get_options_positioning)): quote + default full chain, then aggregate surface metrics from **`data.analytics.options.positioning`** (including GEX, vanna/charm, mispricing, moneyness, pin risk when computable) plus a required **`methodology`** object for transparent client rendering.
