...
The question-driven registry also exposes **`get_options_positioning`** ([market data tools](../analysis-reference/market-data-tools#get_options_positioning)): quote + default full chain, then aggregate surface metrics from **`data.analytics.options.positioning`**.

The default container exposes a composed `MarketDataProvider` (`OptionAnalyticsMarketDataProvider`) that fetches chains and then runs BSM Greek estimation. Vendor-only adapters live under `copinance_os.data.providers`.
...
Default implementation: `QuantLibBsmGreekEstimator` (`copinance_os.data.analytics.options`). European BSM via QuantLib's `AnalyticEuropeanEngine`. See [Options & Greeks](../analysis-reference/options-and-greeks) for full detail.
