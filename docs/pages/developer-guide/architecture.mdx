...
Optional **`interfaces/api`** (FastAPI) lives next to **`interfaces/cli`** and requires the `api` extra (`pip install -e ".[api]"`). There are no `domain/portfolio` or `domain/risk` packages — implement those against `domain.ports` and wire them in `infra/di` like any other adapter.
...
├── interfaces/
│   ├── api/                            # Optional minimal FastAPI app (requires `pip install -e ".[api]"`)
...
| `copinance_os.interfaces.api` | Optional **`create_app`** FastAPI factory (requires `api` extra). |
