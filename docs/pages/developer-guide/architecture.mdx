...
| `copinance_os.research.workflows` | Application-facing **use cases** (analyze, market, profile, fundamentals, backtest). DTOs live in **`domain.models.analysis`** and **`domain.models.market.requests`** (re-exported from workflow modules). |
...
- **Tools:** `Tool`, `ToolSchema`, `ToolParameter`; **bundles:** `ToolBundleFactory` / `ToolBundleContext` (`domain.models.pipeline.tool_bundle_context`, `domain.ports.tool_bundles`)
