...
Optional **`interfaces/api`** (FastAPI) lives next to **`interfaces/cli`**. **`core/events`** is a reserved stub for a future event bus. The shipped tree is below. **There are no first-class `domain/portfolio` or `domain/risk` packages in this repository** (nothing to import or extend under those paths—do not go looking for stubs). Likewise there is no separate top-level **`ai/*`** beyond **`ai/llm`**. If you need portfolio or risk workflows, implement them against **`domain.ports`** and wire them in **`infra/di`** like any other adapter.

## Package tree (source)

Full `src/copinance_os/` layout with **reserved**, **in progress**, **optional**, and **scaffold** labels. For a product-oriented overview, see the repository **README**.

```
copinance_os/
├── ai/
│   └── llm/                            # Providers (Gemini, OpenAI, Ollama, …), analyzer impl, prompts, resources
...
│       ├── bundles/                    # Optional scan targets: modules exporting tool_bundle_factory
...
| `copinance_os.research.workflows` | Application-facing **use cases** (analyze, market, profile, fundamentals, backtest). Analyze and market **DTOs** live in **`domain.models.analysis`** and **`domain.models.market_requests`** (re-exported from workflow modules). |
...
| `copinance_os.ai` | Ships **`llm/`** only today (providers, analyzer, prompts). |
| `copinance_os.core.events` | Reserved — future event bus (package stub). |
...
The global container from **`get_container()`** is a **lazy proxy**: resolving it (or accessors like `container.research_orchestrator()`) constructs the real **`Container`** once per process. Heavy dependencies (providers, executors) are created on first use, not at import time. The CLI keeps **`copinance --help`** fast by **lazy-loading** Typer subcommands (`analyze`, `market`, …) so those modules—and their imports—are not loaded until you run them.

## Dependency injection and startup

Use **`get_container(...)`** from `copinance_os.infra.di`. **Analysis executors** are produced by **`AnalysisExecutorFactory.create_all`** (`core.execution_engine.factory`) inside DI configuration, not by a separate `infra/factories` package. **LLM analyzers** for question-driven mode are created via **`LLMAnalyzerFactory`** (`ai.llm.analyzer_factory`).
