...
Built-in **`GeminiProvider`** and **`OllamaProvider`** subclass **`LLMProvider`** and implement **`_iter_native_text_stream`** for API-native chunking. **`supports_native_text_stream()`** respects **`disable_native_text_stream`**. When adding a new vendor adapter, either:
...
Add your executor in **`infra/factories/analysis_executor.py`** (or build a custom list) and wire it through dependency injection, or override **`container.analysis_executors`** on a **`Container`** you own. Jobs are run via the **`JobRunner`** port (default: finds an executor and runs it). For custom global orchestration (queues, retries), implement **`JobRunner`** and pass it to **`ResearchOrchestrator(job_runner=...)`**—the default **`Container`** does not expose **`job_runner()`** as a method.
