...
class MyCustomExecutor(BaseAnalysisExecutor):
    ...
    async def validate(self, job: Job) -> bool:
        return job.execution_type == "custom"
...
Add your executor to the analysis executors factory or override in the container. Jobs are run via the `JobRunner` port (default implementation finds an executor and runs it); you can override `container.job_runner` with your own implementation for custom orchestration (e.g. queues, retries).
