...
- **Prompt name:** The built-in question-driven analyze flow uses the name `analyze_question_driven`; import `ANALYZE_QUESTION_DRIVEN_PROMPT_NAME` from `copinanceos.infrastructure.analyzers.llm.resources`.
...
- **Custom cache:** `get_container(..., cache_manager=my_cache_manager)`. Your instance must match the `CacheManager` interface (see `copinanceos.infrastructure.cache`). The cache is used for tool outputs and for rendered agent prompts.
