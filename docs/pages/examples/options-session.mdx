...
An options research session: inspect the chain with BSM Greeks, ask positioning questions, run deterministic options analysis, or fetch **aggregate positioning** only via **`copinance analyze positioning <SYMBOL>`** (see [CLI — Analyze aggregate options positioning](../user-guide/cli#analyze-aggregate-options-positioning)). QuantLib must be installed (it is included in the default `make setup`).
