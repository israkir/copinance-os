...
An options research session: inspect the chain with BSM Greeks (including higher-order fields when valid), ask positioning questions, run deterministic options analysis, or fetch **aggregate positioning** only via **`copinance analyze positioning <SYMBOL>`** (see [CLI — Analyze aggregate options positioning](../user-guide/cli#analyze-aggregate-options-positioning)). The aggregate JSON includes GEX, vanna/charm, mispricing, moneyness, and pin-style metrics when the chain supports them. QuantLib must be installed (it is included in the default `make setup`).

Unavailable aggregate inputs are represented as `null` (no fallback `0.0` synthesis). If the requested positioning window has no contracts, positioning raises a validation error instead of returning a silent empty payload.
...
For machine export, use `--json` and consume the top-level `RunJobResult` envelope (`success`, `results`, `error_message`, `report`, `report_exclusion_reason`).
...
```bash
# Overall positioning
copinance analyze options AAPL --question "How is the options market positioned — is there elevated put or call act ivity?"
...
If you only need aggregate deterministic positioning output (no LLM), prefer:

```bash
copinance analyze --json positioning SPY --window near > spy_positioning.json
```

This exports the full `OptionsPositioningResult` under `results`, preserves unavailable metrics as `null`, and fails loudly on empty windows instead of returning silent empty payloads.
...
async def options_analysis():
    container = create_container()  # no LLM needed for deterministic
