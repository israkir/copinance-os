...
- **Python 3.11+**
- For **question-driven (AI) analysis**: an LLM provider (**Gemini**, **OpenAI**, **Anthropic**, or local **Ollama**)
...
async def main():
    ...
    # Question-driven-capable: supply Gemini, OpenAI, Anthropic, or Ollama (see LLMConfig section)
...
**Implemented providers:** **`LLMProviderFactory`** supports **`gemini`**, **`openai`**, **`anthropic`**, and **`ollama`**. Unknown provider names raise **`ValueError`**. `openai` and `anthropic` are core dependencies.
...
llm_config = LLMConfig(
    ...
    api_key="your-api-key",       # required for Gemini, OpenAI, and Anthropic; optional for Ollama
...
|---------------------------------------|----------|
| **`auto`** (default) | Use **native** HTTP/API streaming when the provider supports it (**Gemini**, **OpenAI**, and **Ollama** do; **Anthropic** uses the buffered path for `generate_text_stream`); if native streaming fails, fall back to a single **`generate_text`** call and emit one delta. |
...
- **Disable native streaming** (e.g. model or proxy quirks): set **`provider_config={"disable_native_text_stream": True}`** in **`LLMConfig`**, or construct **`GeminiProvider`** / **`OpenAIProvider`** / **`OllamaProvider`** / **`AnthropicProvider`** with **`disable_native_text_stream=True`**. In **`auto`** mode, the provider then uses the buffered path only.
...
| **Market narrative** | `generate_market_narrative_use_case()` | Literacy-aware prose summary of current conditions (LLM, with deterministic fallback) |
| **Curated questions** | `generate_curated_questions_use_case()` | Ask AI chips from a payload you already fetched |
...
For literacy-aware copy, methodology rendering, and chat UX, use [Financial literacy (clients)](../developer-guide/financial-literacy) and [Agent progress & chat (clients)](../developer-guide/agent-progress-client-integration).
...
- [API Reference](../developer-guide/api-reference) — Ports, container, [deterministic backtest](../developer-guide/api-reference#deterministic-long-only-backtest).
