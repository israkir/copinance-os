...
async def main():
    ...
    # 2. Orchestrator (wraps JobRunner; same path analyze use cases use)
...
| **Orchestration** | `container.research_orchestrator()` | `ResearchOrchestrator` | **Preferred:** `run_job(job, context)` |
| **Job execution (advanced)** | `container.job_runner()` | `JobRunner` | Same port the orchestrator uses; override for custom global routing |
...
### Fundamentals use case — `copinance_os.research.workflows.fundamentals` (market data)
...
| `RunJobResult` | `copinance_os.domain.models.job` | `success: bool`, `results: dict \| None`, `error_message: str \| None` |
...
- **Custom job orchestration:** Implement `JobRunner` (port in `copinance_os.domain.ports.analysis_execution`) and override `container.job_runner`. Prefer keeping `ResearchOrchestrator` as the façade if you only need policy around `run_job`.
...
1. **Implement the runner port** for the flow you want to customize:
   - **Instrument analysis:** `AnalyzeInstrumentRunner` in `research.workflows.analyze` — implement `async def run(self, request: AnalyzeInstrumentRequest) -> RunJobResult`.
   - **Market analysis:** `AnalyzeMarketRunner` in `research.workflows.analyze` — implement `async def run(self, request: AnalyzeMarketRequest) -> RunJobResult`.

2. **Override the runner in the container** so the use case uses your implementation:

   ```python
   from dependency_injector import providers
   from copinance_os.research.workflows.analyze import AnalyzeInstrumentRunner
...
3. **Job-based execution:** If you use jobs (e.g. for queues or batch), you can replace **JobRunner** (e.g. `container.job_runner.override(...)`) so both **`ResearchOrchestrator`** and any direct `job_runner()` consumers use your implementation.

Request/response types live in the same use case modules (e.g. `research.workflows.market`, `research.workflows.analyze`). Use them for type hints and validation.
...
The default **`JobRunner`** (invoked via **`ResearchOrchestrator.run_job`**) finds the executor for the job’s `execution_type`, builds context (e.g. profile), and runs the executor once. You can:
