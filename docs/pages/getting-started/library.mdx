...
For **options-surface / positioning intelligence** (bias, IV/skew/term, flow-style signals, gamma regime, GEX, vanna/charm exposure, mispricing vs BSM fair value, moneyness buckets, pin risk, max pain, implied move, OI clusters) as a **single JSON payload** aligned with the Copinance screener API, use:
...
async def main():
    ...
    # result.model_dump() uses snake_case; use model_dump(by_alias=True) for camelCase API JSON
