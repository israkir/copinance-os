...
- **Data:** `await compute_options_positioning_context(provider, symbol, window)` where `window` is `"near"` or `"mid"`, and `provider` is a **`MarketDataProvider`** (`get_quote`, `get_options_chain`). The usual choice is `container.market_data_provider()` from `get_container(...)`.
...
| `AnalysisReport` | `copinance_os.domain.models.analysis_report` | `summary`, `key_metrics`, `methodology` (`AnalysisMethodology` envelope with `specs` + `data_inputs`) |
