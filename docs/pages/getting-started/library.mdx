...
**From PyPI:** When **`copinance-os`** is published, `pip install copinance-os`. Until then, use **from source** (below).
...
Add to your `requirements.txt` or `pyproject.toml` after **PyPI** release, or depend on a **path / git** URL until then:
```
copinance-os>=0.1.0
# Optional:
...
async def main():
    ...
    # Question-driven-capable: supply Gemini or Ollama (see LLMConfig section)
...
**Implemented providers:** the factory currently supports **`gemini`** and **`ollama`** only. Selecting **`openai`** or **`anthropic`** raises `ValueError` until those backends are added.
...
llm_config = LLMConfig(
    ...
    api_key="your-api-key",     # required for Gemini; optional for Ollama
    model="gemini-1.5-pro",     # optional; provider default if omitted
    temperature=0.7,
    max_tokens=4096,
    base_url=None,              # e.g. Ollama base URL when provider is "ollama"
...
|---------------------------------------|----------|
| **`auto`** (default) | Use **native** HTTP/API streaming when the provider supports it (**Gemini** and **Ollama** do); if native streaming fails, fall back to a single **`generate_text`** call and emit one delta. |
...
- **Disable native streaming** (e.g. model or proxy quirks): set **`provider_config={"disable_native_text_stream": True}`** in **`LLMConfig`**, or construct **`GeminiProvider`** / **`OllamaProvider`** with **`disable_native_text_stream=True`**. In **`auto`** mode, the provider then uses the buffered path only.
...
- **`question_driven_instrument_analysis`** — Instrument scope, tool-using analysis; requires `LLMConfig`. Context includes `question`, optional **`conversation_history`** (list of `{"role","content"}` dicts: alternating user/assistant, ending with assistant), optional options context, `stream`, and `include_prompt`.
- **`question_driven_market_analysis`** — Market scope, tool-using analysis; requires `LLMConfig`. Context includes `question`, optional **`conversation_history`** (same shape), `market_index`, `stream`, and `include_prompt`.
...
- [API Reference](/api-reference/data-providers) — Data provider interfaces and market point coercion helpers.
