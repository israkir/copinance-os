...
You can also run a **standalone research question** at the root (no `analyze` subcommand): `copinance "Your question here"` uses the full tool suite with default macro-style market context—see [CLI — Natural language at the root](../user-guide/cli#natural-language-at-the-root).
...
The model analyzes the selected context using registered tools (market data, fundamentals, regime/macro, and SEC tools when available). Use `--question` and pick `analyze equity`, `analyze options`, or `analyze macro`. See [CLI — Question-driven examples](../user-guide/cli#question-driven-examples) for more ideas.
...
# Deterministic aggregate options positioning (bias, IV metrics, gamma regime, max pain, …)
