...
| `analyze` | Run progressive analysis. Without a question it runs deterministic analysis; with a question it runs tool-using question-driven analysis. Group flags **`--json`** and **`--stream`** apply before the subcommand (`equity`, `options`, `positioning`, `macro`). |
...
For symbol-scoped or instrument flows, use **`copinance analyze equity|options|positioning|macro`** instead.
...
For **`analyze positioning`**, `results` is the deterministic aggregate `OptionsPositioningResult` contract (snake_case fields), including category-level methodology specs plus component-level methodology on blocks such as `iv_metrics`, `delta_exposure`, `vanna_exposure`, `charm_exposure`, `moneyness_summary`, `pin_risk`, and `implied_move_detail`.

## Streaming question-driven output (`--stream`)

Use **`--stream` on the `analyze` group** (before `equity`, `options`, or `macro`; not used for **`positioning`**, which is deterministic-only) or **at the root** before a natural-language question to print **LLM tokens to stdout** as they are generated during question-driven tool loops. **`--json` turns streaming off.** After the run, Rich still prints the same **Analysis** panel (markdown) so you always have a readable block for copy-paste, plus run metadata and the saved file path. Library-oriented streaming options (`LLMConfig`, providers) are described in [Using as a Library — LLM text streaming](../getting-started/library#llm-text-streaming-library--llmprovider).
...
The `analyze` group accepts **`--json`** (see [Machine-readable output](#machine-readable-output---json)) and **`--stream`** (see [Streaming question-driven output](#streaming-question-driven-output---stream)); place them **before** the subcommand (`equity`, `options`, `positioning`, `macro`).
...
Run options analysis. By default it returns deterministic options-chain output (one expiry unless you repeat **`-e` / `--expiration`**). For **single-expiry** deterministic runs (including the default provider expiry), results also include a **`positioning`** object: aggregate surface metrics (bias, IV/skew/term, flow-style scores, gamma regime, GEX, vanna/charm, mispricing, moneyness, pin risk, max pain, implied move, OI clusters) as **snake_case** JSON — plus a required **methodology envelope** (`version`, `computed_at`, `specs`, `data_inputs`) for explainability — see [Options positioning context](../getting-started/library#options-positioning-context). **Multi-expiry** deterministic runs do not attach **`positioning`** (no merged full chain in that path). Add `--question` to let the AI fetch and interpret the relevant options and market data.
...
**Library:** for options-only runs you can set **`positioning_window`** (`"near"` \| `"mid"`) on **`AnalyzeInstrumentRequest`**; the runner passes it as context **`positioning_window`** (default **`near`**).

### Analyze aggregate options positioning

Deterministic **only**: fetches quote + default full chain and prints the same aggregate **`positioning`** payload as the instrument executor (snake_case fields), including extended dealer-style blocks where data supports them. No `--question` / streaming.

Unavailable data is explicit (`null`) rather than synthesized as fallback `0.0` values. If no contracts are available for the selected positioning window/expiry, the command fails with a validation error (no silent empty positioning payload).

```bash
copinance analyze positioning SPY
copinance analyze positioning SPY --window mid
copinance analyze --json positioning SPY -w near
```

**Options:**
- **`--window` / `-w`**: `near` (default) or `mid` — DTE band for which expiries feed the aggregate (`near` 0–21 days, `mid` 21–75; see [Library — Options positioning context](../getting-started/library#options-positioning-context)).
- **`--json`** (group, before `positioning`): machine **`RunJobResult`** JSON.
- `--timeframe`, `--profile-id`, `--no-cache`, `--include-prompt`: same semantics as other **`analyze`** subcommands where applicable.

**Output/export notes:**
- `results` contains full deterministic positioning JSON (not a reduced summary object).
- Missing/insufficient data is explicit (`null`) in JSON; values are not synthesized from fallback defaults.
- If no contracts are available for the selected window, the command exits with validation error instead of exporting a silent empty payload.
...
**Options:**
- `--tool <name>`: Clear cache for a specific tool only (e.g. `get_market_quote`, `get_historical_market_data`, `get_options_chain`, `get_options_positioning`)
