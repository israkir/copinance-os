...
5. For **single-expiry** deterministic runs, computes **aggregate positioning** (bias, IV/skew/term, flow-style scores, gamma regime, max pain, implied move, OI clusters) and adds **`positioning`** to results (**snake_case**). Omitted when multiple expiries are requested in one job.
