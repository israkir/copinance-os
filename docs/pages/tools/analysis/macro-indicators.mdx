...
The macro indicators tool aggregates **observable economic series** (rates, credit, labor, housing, and more) for use in deterministic macro runs and in **question-driven** analysis. Numbers are fetched via **`MacroeconomicDataProvider`** implementations (FRED when configured, yfinance fallbacks where applicable)—the LLM interprets tool output; it does not compute FRED prints.

## Overview

Macro series help describe the **backdrop** for risk assets. This tool groups 47+ indicators into nine categories, with **FRED-first** behavior and **yfinance** proxies when a series is unavailable or when running without a FRED key.

## Key features

- **47+ indicators** across nine categories
- **FRED primary, yfinance secondary** where implemented
- **Structured interpretation** blocks (signals are derived from fetched data, not from model invention)
- **Configurable inclusion** via CLI flags or programmatic `include_*` arguments
...
### 7. Consumer spending and confidence
...
### 9. Advanced economic indicators
...
```bash
# Run full macro/market regime analysis with all indicator categories
...
Create the macro tool from the container's data providers (macro + market). The tool is used internally by analyze executors; you can also build it for direct use:

```python
from copinance_os.infra.di import get_container
from copinance_os.core.pipeline.tools.analysis import create_macro_regime_indicators_tool

container = get_container(
    llm_config=llm_config,  # required for question-driven; optional for macro-only
    fred_api_key="your-fred-api-key",  # optional: enables FRED data
)

fred_provider = container.macro_data_provider()
market_provider = container.market_data_provider()
macro_tool = create_macro_regime_indicators_tool(fred_provider, market_provider)
...
In **question-driven** runs, the same macro tool is included when **`collect_question_driven_tools`** builds the list from a **`ToolBundleContext`** that has both **`macro_data_provider`** and **`market_data_provider`** set (as the default container wiring does when FRED and market data are configured).
...
- No rate limits for analysis use
...
async def comprehensive_research(symbol: str):
    # 1. Get individual instrument analysis
    instrument_analysis = await instrument_tool.execute(symbol=symbol)
    ...
    return generate_recommendation(instrument_analysis, macro_analysis)
```
...
- [Market Regime Detection](market-regime) — Trend, volatility, cycle detection
...
- [Analysis Modes](../../user-guide/analysis-modes) - Integration patterns
