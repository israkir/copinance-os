...
Regime tools classify **price and volatility behavior** from historical series (trend, vol, cycles, VIX, breadth, sector rotation). They live in **`copinance_os.core.pipeline.tools.analysis.market_regime`** and are invoked by executors alongside data-provider tools. Outputs are **structured tool results** suitable for deterministic dashboards and for LLM consumption in question-driven mode—classification logic is rule-based or statistical code, not model hallucination.

They are part of the [Tools](..) suite.

## Overview

Market regimes represent distinct periods characterized by different statistical properties, trends, and behaviors. Regime signals are inputs to risk and allocation decisions; always treat them as **heuristic** and validate against your own assumptions ([Architecture](/developer-guide/architecture)).

Identifying regimes supports:
...
**Example:**
```python
from copinance_os.core.pipeline.tools.analysis import MarketRegimeDetectVolatilityTool
...
**Example:**
```python
from copinance_os.core.pipeline.tools.analysis import MarketRegimeDetectCyclesTool
...
**Example:**
```python
from copinance_os.core.pipeline.tools.analysis import MarketRegimeIndicatorsTool
from copinance_os.data.providers import YFinanceMarketProvider
...
## CLI: What Gets Fetched

When you run `copinance analyze macro`, the CLI fetches data for several symbols. Here’s how it works:

- **Reference index (`--market-index`)**: The symbol you pass (e.g. `SPY`, `QQQ`, `DIA`, `IWM`) is the **reference index**. It is used for:
  - **Trend and volatility regime detection** — that index’s price history is analyzed.
  - **Sector comparison** — “market” return in breadth and rotation is this index’s return.
- **Fixed inputs (same for any reference index)**:
  - **VIX** (^VIX) — always fetched when VIX is included.
  - **11 S&P sector ETFs** (XLK, XLE, XLI, XLV, XLF, XLP, XLY, XLU, XLB, XLC, XLRE) — always fetched when market breadth or sector rotation is included. They are the same whether you choose SPY, QQQ, DIA, or IWM.

So `copinance analyze macro --market-index QQQ` fetches: **QQQ** (reference), **all 11 sector ETFs**, and **^VIX**. That is expected and not “fetching all indexes.”

**Examples:**

```bash
# Default: SPY as reference; 252-day lookback; VIX + breadth + rotation
copinance analyze macro

# QQQ as reference; 90-day lookback (still fetches 11 sector ETFs + VIX)
copinance analyze macro --market-index QQQ --lookback-days 90

# DIA as reference; skip sector rotation (still fetches sector ETFs if breadth is on)
copinance analyze macro --market-index DIA --no-include-sector-rotation

# Question-driven: same data sources; AI answers in natural language
copinance analyze macro --market-index SPY --question "Which sectors are leading and what is the rotation theme?"
```
...
Tools are automatically available in question-driven analysis when a market data provider is configured. The LLM will automatically select and use the appropriate tools based on your question.

**Individual Instrument Analysis:**
```bash
# Analyze trend and volatility for a specific instrument
copinance analyze equity AAPL --question "What is the current market trend and volatility regime?"

# Analyze market cycles
copinance analyze equity TSLA --question "What market cycle phase is this instrument in?"

# Comprehensive regime analysis
copinance analyze equity MSFT --question "Analyze the market regime for this instrument including trend, volatility, and cycle phases"
```

**Market-Wide Indicators:**
```bash
# Get market-wide context with VIX and sector analysis
copinance analyze macro --question "What is the current market sentiment and which sectors are leading?" --market-index SPY

# Analyze market breadth
copinance analyze macro --question "What is the current market breadth and sector rotation?"

# VIX and volatility analysis
copinance analyze macro --question "What is the current VIX level and market fear sentiment?"

# Sector rotation analysis
copinance analyze macro --question "Which sectors are currently outperforming and what rotation theme is active?"
```

**Combined Analysis:**
```bash
# Combine individual instrument and market-wide analysis
copinance analyze equity AAPL --question "Analyze this instrument's regime and compare it to the broader market conditions including VIX and sector rotation"
```
...
**Recommended:** use **`build_data_provider_tool_registry`** so market data, rule-based regime, and fundamental tools share one registry (matches bundle specs in `core.pipeline.tools.discovery.specs`):
...
registry = build_data_provider_tool_registry(
    ...
    fundamental_data_provider=fundamentals_provider,  # optional
)
trend_tool = registry.get("detect_market_trend")
```

**Regime-only** (manual registration):

```python
from copinance_os.core.pipeline.tools import ToolRegistry
from copinance_os.core.pipeline.tools.analysis import create_rule_based_regime_tools

registry = ToolRegistry()
registry.register_many(create_rule_based_regime_tools(provider))
trend_tool = registry.get("detect_market_trend")
```

See [Tools Overview](../index#programmatic-usage) for `collect_question_driven_tools`, `ToolBundleContext`, and plugin entry points.
...
3. **Combined Analysis**: Use all tools together for comprehensive market assessment:
   - Individual instrument tools (trend, volatility, cycles) for instrument-specific analysis
...
```
core/pipeline/tools/analysis/market_regime/
...
4. **Consistent API**: All tools are available via the package's public exports
...
```python
from copinance_os.core.pipeline.tools.analysis.market_regime.base import (
...
- [Macro Economic Indicators](macro-indicators) — Macro categories and usage
- [Data Provider Tools](../data-providers) — Market and fundamental tools
