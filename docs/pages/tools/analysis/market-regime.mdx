...
Tools are automatically available in agent workflows when a market data provider is configured. The LLM will automatically select and use the appropriate tools based on your question.

**Individual Stock Analysis:**
```bash
# Analyze trend and volatility for a specific stock
copinance ask "What is the current market trend and volatility regime?" --symbol AAPL

# Analyze market cycles
copinance ask "What market cycle phase is this stock in?" --symbol TSLA

# Comprehensive regime analysis
copinance ask "Analyze the market regime for this stock including trend, volatility, and cycle phases" --symbol MSFT
```

**Market-Wide Indicators:**
```bash
# Get market-wide context with VIX and sector analysis
copinance ask "What is the current market sentiment and which sectors are leading?" --market-index SPY

# Analyze market breadth
copinance ask "What is the current market breadth and sector rotation?"

# VIX and volatility analysis
copinance ask "What is the current VIX level and market fear sentiment?"

# Sector rotation analysis
copinance ask "Which sectors are currently outperforming and what rotation theme is active?"
```

**Combined Analysis:**
```bash
# Combine individual stock and market-wide analysis
copinance ask "Analyze this stock's regime and compare it to the broader market conditions including VIX and sector rotation" --symbol AAPL
```
